...
options = argparse.ArgumentParser(
    prog="detect", description="Detect objects from camera."
)
...
detector = DetectionLearner(
    model=model, labels=coco_labels()[1:],  # First element is '__background__'
)
...
if len(args.path):
    ...
    for path in args.path:
        ...
        try:
            ...
            sys.stderr.write(
                f"'{imfile}' may not be an image file and "
                + f"will be skipped.\n"
            )
            continue
